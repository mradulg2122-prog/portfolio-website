 ...
 *
 * Changes from the original React source (minimal):
 *   - Removed React imports (useEffect, useRef) and JSX return
 *   - Replaced useRef() with plain `let` variables in closure scope
 *   - Replaced mountRef.current  → container  (querySelector result)
 *   - Replaced isVisibleRef.current → isVisible (plain boolean)
 *   - Replaced rafRef.current   → rafId
 *   - Replaced resizeRafRef.current → resizeRafId
 *   - Replaced webglRef.current → webglInstance
 *   - Replaced resizeObserverRef.current → resizeObserver
 *   - Replaced intersectionObserverRef.current → intersectionObserver
 *   - First useEffect body runs inside initLiquidEther() on DOM ready
 *   - Second useEffect (options sync) not needed without React re-renders
 *   - Cleanup moved to window 'beforeunload' event
...
(function () {
    ...
    function initLiquidEther() {
        var container = document.querySelector('.liquid-ether-container');
        if (!container) return;

        /* ── plain variables replacing React refs ─────────────── */
        var webglInstance = null;
        var rafId = null;
        ...
        /* ── default config (matches React component defaults) ─── */
        var mouseForce = 20;
        var cursorSize = 100;
        var isViscous = false;
        var viscous = 30;
        var iterationsViscous = 32;
        var iterationsPoisson = 32;
        var dt = 0.014;
        var BFECC = true;
        var resolution = 0.5;
        var isBounce = false;
        ...
        var autoDemo = true;
        var autoSpeed = 0.5;
        var autoIntensity = 2.2;
        var takeoverDuration = 0.25;
        var autoResumeDelay = 1000;
        var autoRampDuration = 0.6;

        /* ═══════════════════════════════════════════════════════
           ANIMATION CODE — copied verbatim from React component
           (only ref substitutions applied, logic unchanged)
        ═══════════════════════════════════════════════════════ */
        ...
        class MouseClass {
            ...
            init(cont) {
                ...
                var defaultView = (this.docTarget && this.docTarget.defaultView) ||
        ...
        class AutoDriver {
            ...
            update() {
                ...
                if (this.rampDurationMs > 0) {
                    var tt = Math.min(1, (now - this.activationTime) / this.rampDurationMs);
                    ramp = tt * tt * (3 - 2 * tt);
                }
    ...
    gl_FragColor = vec4(newv, 0.0, 0.0);
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(props) {
                ...
                var csX = props.cursor_size * props.cellScale.x;
                var csY = props.cursor_size * props.cellScale.y;
                var centerX = Math.min(Math.max(Mouse.coords.x, -1 + csX + props.cellScale.x * 2), 1 - csX - props.cellScale.x * 2);
                var centerY = Math.min(Math.max(Mouse.coords.y, -1 + csY + props.cellScale.y * 2), 1 - csY - props.cellScale.y * 2);
        ...
        class Simulation {
            ...
            calcSize() {
                var w = Math.max(1, Math.round(this.options.resolution * Common.width));
                var h = Math.max(1, Math.round(this.options.resolution * Common.height));
        ...
        class WebGLManager {
            constructor(props) {
                ...
                this._onVisibility = () => {
                    if (document.hidden) {
                        this.pause();
                    } else if (isVisible) {   /* isVisible = outer closure variable */
                        this.start();
                    }
                };
            ...
            loop() {
                ...
                this.render();
                rafId = requestAnimationFrame(this._loop);  /* rafRef.current → rafId */
            }
            start() {
                if (this.running) return;
            ...
            pause() {
                this.running = false;
                if (rafId) { cancelAnimationFrame(rafId); rafId = null; }  /* rafRef.current → rafId */
            }
            dispose() {
                try {
        ...
        var io = new IntersectionObserver(function (entries) {
            ...
            isVisible = visible;                            /* isVisibleRef.current → isVisible */
            if (!webglInstance) return;
            if (visible && !document.hidden) webglInstance.start();
            else webglInstance.pause();
        }, { threshold: [0, 0.01, 0.1] });
        io.observe(container);
        intersectionObserver = io;                        /* intersectionObserverRef.current → intersectionObserver */

        /* ResizeObserver — debounced resize */
        var ro = new ResizeObserver(function () {
            if (!webglInstance) return;
            if (resizeRafId) cancelAnimationFrame(resizeRafId);  /* resizeRafRef.current → resizeRafId */
        ...
        resizeObserver = ro;                              /* resizeObserverRef.current → resizeObserver */

        /* Cleanup on page unload (replaces useEffect return cleanup) */
        window.addEventListener('beforeunload', function () {
            if (rafId) cancelAnimationFrame(rafId);
            try { if (resizeObserver) resizeObserver.disconnect(); } catch (e) { void 0; }
            try { if (intersectionObserver) intersectionObserver.disconnect(); } catch (e) { void 0; }
            if (webglInstance) { webglInstance.dispose(); webglInstance = null; }
        });
    }

    /* ── Entry point ──────────────────────────────────────── */
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () { waitForThree(initLiquidEther); });
    } else {
        waitForThree(initLiquidEther);
    }
})();
